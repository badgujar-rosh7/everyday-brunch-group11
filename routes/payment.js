...
router.get('/', async (req,res)=>{
    ...
    res.render('pages/payment',{key:Publishable_Key,amt:amount,descriptionofCartItems,id:req.session.user.userId,getCategory})
    
}else{
    // res.send({error:'you cannot directly access this page'})
    res.render('pages/errors',{errors:'Dang!!!! Your Cart is empty',getCategory,id:req.session.user.userId})
}} else {
        // res.send({error:'login to access the page'})
        res.render('pages/errors',{errors:'Not Authorize to access this page without login',getCategory})
    }
})
// router.get('/', (req,res)=>{
